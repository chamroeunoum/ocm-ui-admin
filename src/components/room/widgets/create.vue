  ...
  <!-- Form edit account -->
    <div class="vcb-pop-create font-ktr">
      <n-modal v-bind:show="show" :on-esc="maskOrEscClick" :on-mask-click="maskOrEscClick" :on-after-enter="initial" transform-origin="center">
        <n-card class="w-10/12 sm:w-3/4 md:w-8/12 lg:w-3/5 xl:w-7/12" title="បន្ថែមសាលប្រជុំ" :bordered="false" size="small">
            ...
            <div class=" mx-auto p-4 flex-wrap">
              <div class="crud-form-panel w-full flex flex-wrap ">
                <n-form v-if="record!=null" 
                  ...
                  ref="formRef"
                >
                  <n-form-item label="ឈ្មោះ" path="name" class="w-6/12 p-1" >
                    <n-input v-model:value="record.name" placeholder="ឈ្មោះ" />
                  </n-form-item>
                  <n-form-item label="កាបរិយាយ" path="desp" class="w-6/12 p-1" >
                    <n-input v-model:value="record.desp" placeholder="កាបរិយាយ" />
                  </n-form-item>
                </n-form>
...
export default {
  ...
  setup(props){
    ...
    function create(){
      if( 
        props.record.name == ""  || props.record.name == null
      ){
        notify.warning({
