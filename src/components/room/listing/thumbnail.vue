      ...
      <Transition name="fade" >
        <div v-if="Array.isArray( table.records.matched ) && table.records.matched.length > 0 " class="vcb-thumbnail mb-12" >
          <div v-for="(record, index) in table.records.matched" :key='index' class="item"  >
            <div class="content image bg-80% bg-cover bg-center bg-no-repeat " :style=" ( record.image != undefined && record.image.length > 0 ? 'background-image: url(' + record.image +');' : '' ) + ' min-height: 16rem; ' " >
              <div class="absolute left-0 top-0 p-1 bg-gray-50 rounded-br-lg text-left " >
                <div class="font-moul">{{ record.name }}</div>
                <div class="text-xxs">{{record.desp }}</div>
              </div>
