  ...
  <div>
    ...
    <div class="flex title-bar border-b border-gray-200">
      ...
      <div class="flex h-10 py-1 title " >
        ...
        <div class="font-moul ml-2 leading-9" >{{ organizationTitle }}</div>
      </div>
      ...
      <div class="flex-grow action-buttons flex-row-reverse flex">
        ...
        <div class="mt-1 mr-2">
          <div v-if="rootOrganization!=null&&rootOrganization.pid!=null&&rootOrganization.parent_node!=null" class=" p-2 border border-gray-300 rounded-sm cursor-pointer hover:border-blue-500 duration-300" @click="$router.push('/organization/'+rootOrganization.pid+'/sub')" >{{ rootOrganization.parent_node.name }}</div>
        </div>
      </div>
              ...
              <div class="flex flex-wrap " >
                ...
                <div class="w-full pb-2 flex mt-4 flex-wrap" >
                  ...
                  <n-tooltip v-if="record.parentNode!=undefined && record.parentNode != null " trigger="hover">
                    <template #trigger>
                      <svg class="cursor-pointer absolute left-8 top-1 w-5 text-blue-500" 
                  ...
                  <n-tooltip v-if="record.parentNode!=undefined && record.parentNode != null " trigger="hover">
                    <template #trigger>
                      <svg @click="showEditModal(record)" class="cursor-pointer absolute left-1 top-1 w-5 text-blue-500" 
...
export default {
  ...
  setup(){
    ...
    const rootOrganization = ref(null)
    const organizationTitle = computed( () => {
      return rootOrganization.value == null ? model.title : "អង្គភាពចំណុះ ៖ " + rootOrganization.value.name
    })

    watch( () => route.params.rootId, (newValue, oldValue) => {
      setCurrentOrganizationId( newValue )
    })
    ...
    function getRecords(){
      ...
      store.dispatch(model.name+'/list',{
        ...
        rootOrganization.value = res.data.root
        closeTableLoading()
      }).catch( err => {
        console.log( err )
      })
    ...
    function getOrganizations(){
      store.dispatch('organizations/compact',{
        page: 1 ,
        perPage: 1000 ,
        search: '' ,
        id: 163
    ...
    function deactivateRecord(id){
      store.dispatch('organizations/deactivate',{id:id,active:0})
      getRecords()
    }
    function activateRecord(id){
      store.dispatch('organizations/activate',{id:id,active:1})
      getRecords()
    }
    /**
     * Initial the data
