          ...
          <div class="card relative border border-gray-300 rounded p-8 -my-3 -mx-4">
            <div v-if="record.person != undefined " class="flex flex-initial" >
              ...
              <div class="flex-none p-4 " >
                ...
                <!-- Picture -->
                <div class="w-24 h-24 lg:w-40 lg:h-40 md:w-32 md:h-32  mx-auto bg-center bg-no-repeat bg-white bg-cover rounded border border-gray-200 " :style=" 'background-image: url(' + (record.avatar_url!=''? record.avatar_url:'/src/assets/logo.svg') +');' " ></div>
                <!-- Officer ID -->
...
export default {
  ...
  setup(props){
    ...
    const userCountesies = computed( () => {
      return props.record.person != undefined && Array.isArray( props.record.person.countesies ) ? props.record.person.countesies.map( ( c ) => c.name ).join(', ') : 'ok'
    })
